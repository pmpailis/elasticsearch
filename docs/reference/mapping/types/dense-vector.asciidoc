...
`hamming_distance`:::
Computes a similarity score based on the hamming distance of two vectors. This is only available for byte vectors,
and should throw for floating point values. The `hamming distance` of two bytes is defined as the population count of
their `XOR` results, i.e. how many bits are different in the binary representation of all bytes.
In order to convert this distance to a similiraty score, we normalize it as `1 / (1 + hammingDistance)`
