...
import org.elasticsearch.search.retriever.CombineRetrieverBuilder;
...
/**
 * A {@link CombineRetrieverBuilder} that combines top documents from the sub-retrievers using
 * Reciprocal Rank Fusion (RRF).
 */
public final class RRFRetrieverBuilder extends CombineRetrieverBuilder<RRFRetrieverBuilder> {
    ...
    static final ConstructingObjectParser<RRFRetrieverBuilder, RetrieverParserContext> PARSER = new ConstructingObjectParser<>(
        ...
        args -> {
            ...
            List<RetrieverSource> childRetrieverSources = childRetrievers.stream()
                .map(r -> new RetrieverSource(r, null))
            ...
            return new RRFRetrieverBuilder(childRetrieverSources, rankWindowSize, rankConstant);
        }
    );
    ...
    public RRFRetrieverBuilder clone(RRFRetrieverBuilder original, List<RetrieverSource> newRetrievers) {
        return new RRFRetrieverBuilder(newRetrievers, original.windowSize, original.rankConstant);
    }
    ...
    public RRFRankDoc[] combineQueryPhaseResults(List<ScoreDoc[]> rankResults) {
        ...
        Map<RankDoc.RankKey, RRFRankDoc> docsToRankResults = Maps.newMapWithExpectedSize(windowSize);
        int index = 0;
    ...
    public void doToXContent(XContentBuilder builder, Params params) throws IOException {
        ...
        builder.field(RANK_WINDOW_SIZE_FIELD.getPreferredName(), windowSize);
