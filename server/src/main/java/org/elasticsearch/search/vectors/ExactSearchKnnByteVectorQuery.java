...
public class ExactSearchKnnByteVectorQuery extends ESKnnByteVectorQuery {

    private final VectorSimilarityFunction vectorySimilarityFunction;

    public ExactSearchKnnByteVectorQuery(String field, byte[] target, int k, Query filter, VectorSimilarityFunction similarityFunction) {
    ...
    protected TopDocs getLeafResults(LeafReaderContext ctx, Weight filterWeight) throws IOException {
        ...
        while ((doc = docIdSetIterator.nextDoc()) != DocIdSetIterator.NO_MORE_DOCS) {
            ...
            byte[] storedVector = Arrays.copyOfRange(fieldDocValues.binaryValue().bytes, 0, 96); // due to magnitude
